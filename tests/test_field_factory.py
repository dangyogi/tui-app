...
from tui_app.field import (
    read_only_field, editable_field,
...
def test_single_line_shared_read_only(app):
    ...
    assert shared.field_class is read_only_field
...
def test_single_line_shared_editable_keeps_alignment(app):
    ...
    assert shared.field_class is editable_field
...
def test_field_for_pulls_text_and_attr(app):
    ...
    assert isinstance(f, editable_field)
    assert f.text == "42"
...
def test_field_for_read_only(app):
    ...
    assert isinstance(f, read_only_field) and not isinstance(f, editable_field)
    assert f.text == "eggs"
...
def test_from_field_preserves_edit_state(app):
    ...
    old = editable_field(1, "hello world", shared, begin_y=5)
    old.changed = True
    old.position = 3
    old.selection_len = 2
    new = shared.from_field(old, begin_y=9, screen_key=1)
    assert isinstance(new, editable_field)
